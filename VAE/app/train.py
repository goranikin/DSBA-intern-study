...
if __name__ == "__main__":
    ...
    model = load_model(input_dim=x_dim, hidden_dim=hidden_dim, latent_dim=latent_dim, is_training=True)
    model.to(device)

    optimizer = Adam(model.parameters(), lr=lr)

    trainer = VaeTrainer(model=model, optimizer=optimizer, x_dim=x_dim, loss_fn=loss_function, epochs=epochs, device=device, train_loader=train_dataloader, test_loader=test_dataloader)
